...
同频 Clean 是一个 Android + Node.js 的共同听歌桥接工具。手机端读取当前媒体状态并上传到用户自己的房间服务器；兼容远程 MCP 的 AI 客户端或网页遥控器可以读取歌曲、进度和歌词，并发送播放控制命令。

> **公开测试版提示**：后台领取指令和歌词增强仍可能受不同手机系统、QQ 音乐版本与省电策略影响。请勿把房间密钥发到公开区域。
...
App 中填写基础地址，不要添加 `/mcp`；连接 AI 客户端时使用：
...
打开仓库的 **Actions → Build Tongpin Clean → 最新成功运行 → Artifacts**，下载 `tongpin-clean-1.1.2-apk`，解压并安装其中的 `app-debug.apk`。
...
6. 需要增强歌词时，可主动开启 QQ 音乐歌词读取；OCR 只作为可选兜底。

## MCP 工具

- `create_room`：创建房间。
- `get_room`：读取歌曲、进度、歌词、播放状态和命令结果。
...
QQ 音乐界面文字节点（用户主动授权）
  ↓ 仍读不到
本地中文 OCR（用户主动开启，Android 11+）
```

界面读取与 OCR 只针对 QQ 音乐。服务端只接收当前句、下一句、来源与识别时间，不接收屏幕截图。
...
提交 Issue 时请附上手机品牌、Android 版本、QQ 音乐版本、App 版本、复现步骤和已遮挡敏感信息的截图。
